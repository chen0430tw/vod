"""VOD-real-image-rgb64 — Stage 1 RGB 64×64 substrate smoke (Codex 协议).
...
def build_batched_targets(args, device, rng):
    """RGB 64×64 real-image smoke. Returns image shape (B, H, W, 3)."""
    ...
    # video stub: (B, T=1, H, W, 3) — same RGB tensor with T=1
...
def train_model(args, device, batched_targets):
    ...
    if args.use_field_lift:
        ...
        print(f"[train] Stage-1 RGB64: enc/dec heads replaced "
              f"(RGBFieldLift K=8 per RGB → Linear(24, {args.channels}))",
              flush=True)
