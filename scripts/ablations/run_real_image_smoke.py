"""VOD-v16-real-image-smoke — does the v16 substrate hold on real images?
...
def _load_cifar10_grayscale(args, rng) -> np.ndarray:
    """Return (B, H, W) float32 in [-1, 1]. Uses HF datasets parquet
    (uoft-cs/cifar10) — no trust_remote_code needed in datasets v4+."""
    from datasets import load_dataset
    print(f"[data] loading CIFAR-10 from uoft-cs/cifar10 (HF parquet) ...",
          flush=True)
    ...
    for i, di in enumerate(idx):
        img = ds[int(di)]["img"]   # PIL Image RGB 32×32
        # grayscale via luminance + resize via PIL (NEAREST=fastest;
        # CIFAR is already 32×32 so resize is no-op when image_size=32)
        img = img.convert("L")
        if img.size != (args.image_size, args.image_size):
            img = img.resize((args.image_size, args.image_size))
        arr = np.asarray(img, dtype=np.float32) / 127.5 - 1.0   # → [-1, 1]
        out[i] = arr
    print(f"[data] CIFAR-10 grayscale loaded: shape={out.shape} "
...
def build_batched_targets(args, device, rng):
    """Real-image smoke: load CIFAR-10 (HF) or local imagefolder."""
    if args.dataset == "cifar10":
        try:
            images_np_arr = _load_cifar10_grayscale(args, rng)
        except Exception as e:
            print(f"[data] HF cifar10 unavailable ({type(e).__name__}: {e})")
            print(f"[data] falling back to --data-dir {args.data_dir!r}")
            images_np_arr = _load_local_imagefolder(args, rng)
...
def main():
    ...
    p.add_argument("--out", default="generated/real_image_smoke")
    p.add_argument("--report-out", default="prototype/real_image_smoke_result.json")
    p.add_argument("--md-out", default="prototype/real_image_smoke_report.md")
