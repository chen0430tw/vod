...
def main():
    ...
    p.add_argument("--w-weak", type=float, default=0.5,
                   help="Fix R (v11): auxiliary weak decoder loss weight. "
                        "0 disables. Forces encoder to spread info across "
                        "all channels (anti posterior collapse).")
    ...
    payload = {
        ...
        "version": "v11: A-O + P(LDM-scale) + Q(v-pred+zsnr) + R(weak-decoder)",
        "device": str(device),
    ...
    md = [
        "# Unconditional Sample Fidelity v11 Report\n\n",
        f"**Date**: {payload['date']}\n",
