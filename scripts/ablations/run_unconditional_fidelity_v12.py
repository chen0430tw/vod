...
class WeakDecoder(torch.nn.Module):
    """Fix R' (v12): deeper weak decoder with nonlinear bottleneck.
    Original v11 was Linear(C→1) — a delta-collapsed channel could
    pass through if other channels compensate. With GELU bottleneck,
    a constant channel input → constant pre-GELU → constant post-GELU
    contribution → cannot recover spatial structure unless the channel
    itself carries spatial info. Forces all channels to be informative."""
    def __init__(self, channels, hidden_mult=4):
        super().__init__()
        h = channels * hidden_mult
        self.net = torch.nn.Sequential(
            torch.nn.Linear(channels, h, bias=True),
            torch.nn.GELU(),
            torch.nn.Linear(h, 1, bias=True),
        )

    def forward(self, latent):
        # latent: (B, T, H, W, C) → (B, H, W) — take first frame
        return self.net(latent[:, 0]).squeeze(-1)
...
def main():
    ...
    p.add_argument("--no-zero-terminal-snr", dest="zero_terminal_snr",
                   action="store_false")
    p.add_argument("--w-weak", type=float, default=1.5,
                   help="Fix R (v11/v12): auxiliary weak decoder weight. "
                        "v11 default 0.5 → v12 1.5 (3×). 0 disables.")
    p.add_argument("--w-chvar", type=float, default=10.0,
                   help="Fix R'' (v12): channel-variance-floor weight. "
                        "Penalty = w_chvar * mean(max(0, target_min_std - "
                        "per_ch_std)^2). 0 disables.")
    p.add_argument("--target-min-std", type=float, default=0.20,
                   help="Fix R'' (v12): minimum per-channel std target. "
                        "Below this triggers chvar penalty. v11 had "
                        "ch1=0.117 — set 0.20 to push it well above floor.")
    ...
    payload = {
        ...
        "version": "v12: A-O + P(LDM) + Q(v-pred+zsnr) + R'(deep-weak) + R''(ch-var-floor)",
        "device": str(device),
    ...
    md = [
        "# Unconditional Sample Fidelity v12 Report\n\n",
        f"**Date**: {payload['date']}\n",
