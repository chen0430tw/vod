"""Unconditional sample fidelity v14 — encoder weight floor (Fix R*).

v11 left ch1 std at 0.117 (no collapse but weak). v12/v13 attempted
indirect fixes (deeper weak decoder, channel-variance loss, per-channel
aux) — all REGRESSED Fix R because they target downstream symptoms.

v14 attacks the architectural root cause directly:

  Fix R* (NEW in v14) — encoder weight magnitude floor
      For nn.Linear(1, C) encoders (enc_image, enc_video):
          channel_std[c] = |weight[c, 0]| × std(input)
      So channel collapse <=> weight[c, 0] → 0.
      Add a hinge penalty:
          L_encw = mean over c of max(0, floor - |weight[c, 0]|)^2
      With floor=0.3 every channel weight magnitude is bounded above 0.3,
      guaranteeing every channel std >= 0.3 × std(image).

      Source: weight magnitude lower-bound regularization is a
      classical InfoMax-style constraint (Bell & Sejnowski 1995 for
      ICA, plus standard "spectral / Frobenius bounded" weight reg in
      modern training); applied here as a per-row weight floor.

      This is a TRAINER-SIDE regularizer — it does NOT modify
...
class WeakDecoder(torch.nn.Module):
    ...
    def forward(self, latent):
        # latent: (B, T, H, W, C) → (B, H, W) — take first frame, project
    ...
    for ep in range(start_ep, args.epochs):
        ...
        for s in range(steps_per_epoch):
            ...
            with autocast_ctx:
                ...
                # Fix R* (v14): encoder weight magnitude floor.
                # nn.Linear(1, C).weight has shape (C, 1). |weight[c, 0]|
                # directly bounds the spatial std of latent channel c.
                L_encw = torch.tensor(0.0, device=device)
                if args.w_encw > 0:
                    enc_image_w = m.enc_image.weight.abs().squeeze(-1)  # (C,)
                    enc_video_w = m.enc_video.weight.abs().squeeze(-1)  # (C,)
                    deficit_img = torch.clamp(args.encw_floor - enc_image_w, min=0.0)
                    deficit_vid = torch.clamp(args.encw_floor - enc_video_w, min=0.0)
                    L_encw = ((deficit_img ** 2).mean() + (deficit_vid ** 2).mean()) / 2
...
def main():
    ...
    p.add_argument("--w-encw", type=float, default=50.0,
                   help="Fix R* (v14): encoder weight magnitude floor "
                        "penalty weight. 0 disables.")
    p.add_argument("--encw-floor", type=float, default=0.3,
                   help="Fix R* (v14): minimum |weight| for each row of "
                        "enc_image / enc_video. 0.3 → channel std >= "
                        "0.3 × std(image).")
    ...
    payload = {
        ...
        "version": "v14: A-O + P(LDM) + Q(v-pred+zsnr) + R(weak) + R*(enc-w-floor)",
        "device": str(device),
    ...
    md = [
        "# Unconditional Sample Fidelity v14 Report\n\n",
        f"**Date**: {payload['date']}\n",
