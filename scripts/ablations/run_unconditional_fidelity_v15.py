...
class FieldLift(torch.nn.Module):
    """Lift a scalar field (..., 1) into 8 field features:
       [I, smooth3, smooth5, I-smooth3, Sobel_x, Sobel_y, laplacian, local_energy]
    Convolutions over the last two spatial dims.

    Replaces the (..., 1) → (..., C) Linear at the encoder entrance.
    The trailing nn.Linear(8, C) is part of FieldLift+Linear sequential.
    """
    def __init__(self):
        super().__init__()
        # Smooth kernels (3-tap and 5-tap, separable averaging)
        ...
        laplacian = torch.tensor([[0., 1., 0.],
                                  [1., -4., 1.],
                                  [0., 1., 0.]])
        # Stack (out_C, in_C, kH, kW). All operate on a single-channel image.
        # Smooth3/Sobel/Lap use 3×3, smooth5 uses 5×5 — handled by separate calls.
        self.register_buffer("k3", k3.view(1, 1, 3, 3))
        self.register_buffer("k5", k5.view(1, 1, 5, 5))
        self.register_buffer("sx", sobel_x.view(1, 1, 3, 3))
        self.register_buffer("sy", sobel_y.view(1, 1, 3, 3))
        self.register_buffer("lap", laplacian.view(1, 1, 3, 3))

    def forward(self, x_last1):
        """Input shape (..., 1). Output shape (..., 8). Last-2-dim spatial."""
        # Squeeze trailing singleton, ensure shape (..., H, W)
        x = x_last1.squeeze(-1)               # (..., H, W)
        original_shape = x.shape
        H, W = original_shape[-2], original_shape[-1]
        # Flatten leading dims into batch for conv2d
        x_flat = x.reshape(-1, 1, H, W).contiguous()
        # Apply convolutions with padding to preserve H, W
        smooth3 = F.conv2d(x_flat, self.k3, padding=1)
        smooth5 = F.conv2d(x_flat, self.k5, padding=2)
        sobel_x = F.conv2d(x_flat, self.sx, padding=1)
        sobel_y = F.conv2d(x_flat, self.sy, padding=1)
        laplac  = F.conv2d(x_flat, self.lap, padding=1)
        hf_residual = x_flat - smooth3
        # Local energy = smooth3(x²)
        local_energy = F.conv2d(x_flat * x_flat, self.k3, padding=1)
        # Stack into (-1, 8, H, W) → (..., H, W, 8)
        feats_8 = torch.stack([
            x_flat.squeeze(1),
            smooth3.squeeze(1),
            smooth5.squeeze(1),
            hf_residual.squeeze(1),
            sobel_x.squeeze(1),
            sobel_y.squeeze(1),
            laplac.squeeze(1),
            local_energy.squeeze(1),
        ], dim=-1)                            # (-1, H, W, 8)
        # Restore leading dims: replace last (1) with (8) in original_shape
        out_shape = original_shape + (8,)
        return feats_8.reshape(out_shape)


class FieldLiftedEncoder(torch.nn.Module):
    """Drop-in replacement for nn.Linear(1, C) used by enc_image / enc_video.
    Composes FieldLift (1 → 8) with Linear(8, C). Forward signature
    matches the original Linear: input (..., 1) → output (..., C)."""
    def __init__(self, channels):
        super().__init__()
        self.lift = FieldLift()
        self.proj = torch.nn.Linear(8, channels, bias=True)

    def forward(self, x):
        feats = self.lift(x)              # (..., 8)
        return self.proj(feats)           # (..., C)
...
def main():
    ...
    payload = {
        ...
        "version": "v15: A-O + P + Q + R + S(field-feature lift before encoder)",
        "device": str(device),
    ...
    md = [
        "# Unconditional Sample Fidelity v15 Report\n\n",
        f"**Date**: {payload['date']}\n",
