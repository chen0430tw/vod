"""Unconditional sample fidelity v16 — DCT-orthogonal lift + sanity gate.

v15 used 8 hand-crafted features [I, smooth3, smooth5, HF, sx, sy, lap,
local_energy]. They are NOT mutually orthogonal (smooth3/smooth5 are
low-pass copies of I, etc) → channel correlation max=0.992 still fails
Codex criterion #2.

v16 fixes by using a real orthogonal basis: 2D DCT-II.

  Fix S' (NEW in v16) — DCT-II 2D field lift
      Apply 2D DCT to a 3×3 (or kxk) local neighborhood of each pixel,
      take K lowest-frequency coefficients (K=8). DCT-II is mathematically
      orthogonal: <φ_i, φ_j> = 0 for i ≠ j. Therefore lifted features
      are decorrelated by construction (modulo non-Gaussian image
      statistics).

      Implementation: pre-compute 8 DCT basis functions of size 3×3,
      apply as conv2d filters with padding=1. Then Linear(8, C).

      No hand-crafted feature selection — DCT is a canonical
      orthogonal basis from signal processing.

  Sanity gate (NEW in v16): before any GPU launch, run a CPU smoke
      test that prints channel correlation of L_ref after a few
      training steps. If max |corr| > 0.5, abort. Saves wasted GPU
      time.

v15 docs follow.

Unconditional sample fidelity v15 — field-feature lift (Fix S).

v11→v14 all attacked symptom: ch1 std=0.117 → tried weak decoder, ch-var
floor, per-channel aux, encoder weight floor. NONE worked because the
ROOT CAUSE is upstream of the symptom (per Codex review):

    enc_image = nn.Linear(1, C)
    u_c(y,x) = w_c × image(y,x) + b_c     ← all C channels are scaled
                                            copies of the SAME scalar
                                            field. Channel correlation
                                            is structurally ~1. No
                                            penalty can manufacture
                                            independent information.

  Fix S (NEW in v15) — multi-scale field-feature lift before encoder
      Source: classical scale-space / Sobel / image-pyramid feature
      extraction (Lindeberg 1994 scale-space; Marr-Hildreth Laplacian).
      Lift the scalar pixel into 8 distinct field features:
          [identity, smooth3(I), smooth5(I), I − smooth3(I) (HF
           residual), Sobel-x, Sobel-y, Laplacian, local energy]
      Then enc_image: Linear(8, C). Now each latent channel is a
      learned linear combination of 8 INDEPENDENT field features →
      structurally non-redundant, channel correlation no longer 1.

      Implemented as a FieldLift module that wraps the input before
      passing to the original NativeVOD encoder. enc_image is REPLACED
      at trainer side from Linear(1, C) → Sequential(FieldLift,
      Linear(8, C)). NativeVOD class itself is NOT modified — this is
      a callsite swap of a module attribute.

  Removes (revert to v11 base):
      Fix R' (deeper weak decoder)         — v12 mistake
      Fix R'' (chvar floor)                — v12 misdirection
      Fix R''' (per-channel aux)           — v13 mistake
      Fix R*  (soft encoder weight floor)  — v14 surface fix
      Fix R** (hard projection)            — v15-attempt-1 also surface

  Stops adding loss. Fixes representation.
...
class FieldLift(torch.nn.Module):
    ...
    Replaces the (..., 1) → (..., C) Linear at the encoder entrance.
    The trailing nn.Linear(K, C) is part of FieldLift+Linear sequential.
    """
    ...
    def forward(self, x_last1):
        ...
        x = x_last1.squeeze(-1)               # (..., H, W)
        original_shape = x.shape
        H, W = original_shape[-2], original_shape[-1]
        x_flat = x.reshape(-1, 1, H, W).contiguous()
        # K parallel conv2d (one per DCT basis). Padding to preserve H, W.
        # Kernel pad: kH//2, kW//2.
        out = F.conv2d(x_flat, self.kernels,
                       padding=(self.kH // 2, self.kW // 2))   # (-1, K, H, W)
        # → (-1, H, W, K)
...
class FieldLiftedEncoder(torch.nn.Module):
    """v16: pure DCT lift, NO trailing Linear projection.
    Latent IS the K DCT coefficients (K = channels). Channels stay
    orthogonal because Linear(K, C) was the v15 mixer that destroyed
    orthogonality (sanity gate caught it: max corr 0.76 after 50 steps).
    A small per-channel scale + bias is allowed (decoupled — does not
    couple channels) for the model to learn channel-level affine."""
    def __init__(self, channels):
        super().__init__()
        self.lift = FieldLift(K=channels)
        # Per-channel scale & bias only (no cross-channel mixing).
        self.scale = torch.nn.Parameter(torch.ones(channels))
        self.bias = torch.nn.Parameter(torch.zeros(channels))

    def forward(self, x):
        feats = self.lift(x)                          # (..., K=C)
...
def train_model(args, device, batched_targets):
    ...
    # Fix S (v15): replace scalar-input encoders with field-feature lift.
    # Linear(1, C) → FieldLift + Linear(8, C). 8 distinct field features
    # (smooth3/5, HF residual, Sobel-xy, Laplacian, local energy) give
    # encoder genuine independent inputs per channel.
    if args.use_field_lift:
        m.enc_image = FieldLiftedEncoder(channels=args.channels).to(device)
        m.enc_video = FieldLiftedEncoder(channels=args.channels).to(device)
        print(f"[train] Fix S: enc_image / enc_video replaced with "
              f"FieldLift(8) + Linear(8 → {args.channels})", flush=True)
...
def main():
    ...
    if args.use_field_lift:
        untrained_model.enc_image = FieldLiftedEncoder(channels=args.channels).to(device)
        untrained_model.enc_video = FieldLiftedEncoder(channels=args.channels).to(device)
