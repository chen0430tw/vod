"""Unconditional sample fidelity v6 — large data + cosine LR.
...
All six fixes live in this trainer script — no changes to vod_minimal/.
"""
...
def train_model(args, device, batched_targets):
    ...
    # Fix F: cosine LR schedule. v5 plateau'd at L_diff≈0.145 from
    # ep 1800-2000 with constant lr=1e-4. Cosine decay to 1% of base lr
    # over the full run lets the optimiser walk into the basin without
    ...
    for ep in range(args.epochs):
        ...
        # Fix D: detach the latent before diffusion loss. Encoder is
        # only updated by L_recon below; diffusion gradient cannot push
        # the encoder into trivial collapse (latent → constant), which
        # is what kills training under joint optimisation. This mirrors
        # the standard LDM recipe (pretrained VAE → freeze → diffusion
        # on latent), but with a one-line detach instead of a separate
        # pretraining stage.
        ...
        # Encoder gradient flows ONLY through this path.
...
def main():
    ...
    p.add_argument("--out", default="generated/diffusion_samples_v6")
    p.add_argument("--report-out", default="prototype/unconditional_fidelity_result_v6.json")
    p.add_argument("--md-out", default="prototype/unconditional_fidelity_report_v6.md")
