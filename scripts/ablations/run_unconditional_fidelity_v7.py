"""Unconditional sample fidelity v7 — bf16 autocast + cudnn.benchmark.
...
All seven fixes live in this trainer script — no changes to vod_minimal/.
"""
...
def train_model(args, device, batched_targets):
    ...
    for ep in range(args.epochs):
        ...
        with autocast_ctx:
            x_0_live = batched_encode(m, batched_targets)
            # cast to fp32 for stats (EMA needs precision)
...
def main():
    ...
    p.add_argument("--out", default="generated/diffusion_samples_v7")
    p.add_argument("--report-out", default="prototype/unconditional_fidelity_result_v7.json")
    p.add_argument("--md-out", default="prototype/unconditional_fidelity_report_v7.md")
    ...
    payload = {
        ...
        "version": "v7: A+B+C+D+E+F + G (bf16 autocast + cudnn.benchmark)",
        "device": str(device),
