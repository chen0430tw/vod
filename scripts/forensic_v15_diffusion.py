...
def main():
    ckpt_path = "runs/v15_ckpt/v8_latest.pt"
    ...
    # v15: swap to FieldLiftedEncoder before loading state dict
    import sys as _sys
    _sys.path.insert(0, str(Path(__file__).resolve().parent / "ablations"))
    from run_unconditional_fidelity_v15 import FieldLiftedEncoder
    m.enc_image = FieldLiftedEncoder(channels=args_dict["channels"]).to(device)
    m.enc_video = FieldLiftedEncoder(channels=args_dict["channels"]).to(device)
    ...
    # Codex criterion #2: channel correlation should NOT be all near 1
    # If all channels are scaled copies of same scalar field, abs(corr) ≈ 1
    L_flat = L_ref.reshape(-1, L_ref.shape[-1]).cpu().numpy()  # (N, C)
    corr_mat = np.corrcoef(L_flat.T)  # (C, C)
    off_diag = corr_mat[np.triu_indices_from(corr_mat, k=1)]
    print(f"\n[Codex check - channel correlation]")
    print(f"  off-diagonal |corr|: max={float(np.abs(off_diag).max()):.3f}  "
          f"mean={float(np.abs(off_diag).mean()):.3f}")
    print(f"  → {'PASS (not all near 1)' if np.abs(off_diag).max() < 0.99 else 'FAIL'}")
    ...
    out_json = out_dir / "v15_diffusion_forensic.json"
